							...
							if (!empty($displayRecipients)):
								...
								foreach ($displayRecipients as $recipientId) {
									if (isset($arResult["USERS"][$recipientId])) {
										$recipientNames[] = '<span class="b24-recipient-link" data-user-id="' . $recipientId . '">' . htmlspecialchars($arResult["USERS"][$recipientId]['FULL_NAME']) . '</span>';
									}
								}
...
<script>
	...
	function insertImageToCommentEditor(postId, imageUrl) {
		...
		// Fallback - вставляем в textarea если она есть
