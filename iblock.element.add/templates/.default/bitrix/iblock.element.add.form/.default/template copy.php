...
// Инициализация ELEMENT_PROPERTIES для предотвращения ошибок
if (!isset($arResult["ELEMENT_PROPERTIES"]) || !is_array($arResult["ELEMENT_PROPERTIES"])) {
    $arResult["ELEMENT_PROPERTIES"] = [];
}
	...
	<table class="data-table" style="width: 90%">
		...
		<tbody>
			<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):
				// Инициализация свойства в ELEMENT_PROPERTIES если его нет
				if (!isset($arResult["ELEMENT_PROPERTIES"][$propertyID]) || !is_array($arResult["ELEMENT_PROPERTIES"][$propertyID])) {
					$arResult["ELEMENT_PROPERTIES"][$propertyID] = [];
				}
			?>
					<tr>
						...
						{
							$propertyValuesCount = isset($arResult["ELEMENT_PROPERTIES"][$propertyID]) && is_array($arResult["ELEMENT_PROPERTIES"][$propertyID]) 
								? count($arResult["ELEMENT_PROPERTIES"][$propertyID]) 
								: 0;
							$inputNum = ($arParams["ID"] > 0 || count($arResult["ERRORS"]) > 0) ? $propertyValuesCount : 0;
						...
						switch ($INPUT_TYPE):
							case "USER_TYPE":
								for ($i = 0; $i<$inputNum; $i++)
								{
									...
									{
										$value = intval($propertyID) > 0 && isset($arResult["ELEMENT_PROPERTIES"][$propertyID][$i]) 
											? $arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["~VALUE"] 
											: (intval($propertyID) <= 0 ? $arResult["ELEMENT"][$propertyID] : "");
										$description = intval($propertyID) > 0 && isset($arResult["ELEMENT_PROPERTIES"][$propertyID][$i]) 
						...
						<textarea cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
							...
							case "F":
								for ($i = 0; $i<$inputNum; $i++)
								{
									...
									$valueId = isset($arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUE_ID"]) 
										? $arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUE_ID"] 
										: $i;
									?>
						<input type="hidden" name="PROPERTY[<?=$propertyID?>][<?=$valueId?>]" value="<?=$value?>" />
						<input type="file" size="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>"  name="PROPERTY_FILE_<?=$propertyID?>_<?=$valueId?>" /><br />
									<?

									if (!empty($value) && is_array($arResult["ELEMENT_FILES"][$value]))
									{
										?>
					<input type="checkbox" name="DELETE_FILE[<?=$propertyID?>][<?=$valueId?>]" id="file_delete_<?=$propertyID?>_<?=$i?>" value="Y" /><label for="file_delete_<?=$propertyID?>_<?=$i?>"><?=GetMessage("IBLOCK_FORM_FILE_DELETE")?></label><br />
										<?
